...
def scan_rms(filename):
    ...
    for rms in np.arange(1.3, 2.6, 0.2):
        print(rms)
        tmp = test_blob(sky_, 108, rms_factor=rms, filename=filename)
        lst.append(tmp)
    lst = np.asarray(lst)
    with open(filename + '_rms_pickle_high_2', 'wb') as f:
        pickle.dump(lst, f)
...
#test_vote('../data/full_DR12_mock.fits')
...
scan_rms('../models/cf_set_6_mock_1_108')
scan_rms('../models/cf_set_6_mock_2_108')
scan_rms('../models/cf_set_6_mock_3_108')
scan_rms('../models/cf_set_6_mock_4_108')
